...
6. ~~**Accesibilidad completa**~~ — **HECHA el 19-ago-2026**, y la decisión de aparcarla estuvo bien tomada en su momento: primero había que arreglar las cifras. De 0 a 157 etiquetas (123 en pantallas, 33 en componentes, 1 en navegación), con un guardarraíl —`NadaInteractivoSinNombre`— que rompe el build si aparece un pulsable mudo.

   Lo que no se esperaba: al etiquetar salieron **tres defectos de uso** que nada tenían que ver con lectores de pantalla. El campo del MONTO y el buscador pasaban `label=""` y el átomo generaba una etiqueta vacía, así que los dos campos más usados de la app no existían para quien no ve la pantalla. Un botón medía 32×32, por debajo del mínimo de 44 para acertar con el pulgar. Y varios estados —deshabilitado, logro conseguido— solo se distinguían por opacidad.

   Etiquetar obliga a decir en voz alta qué hace cada cosa, y ahí es donde se nota que algo no se puede usar.
